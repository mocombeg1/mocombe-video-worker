...
so avatar + generative renders work identically whether hosted on RunPod or run here on the A100.

Set MODEL_CACHE_DIR before running (bootstrap_a100.sh does this) so weights resolve on local disk
instead of the RunPod /runpod-volume default.

  # talking-avatar (clone a face + speak a script; optional voice clone from a sample)
  python run_local.py avatar --reference_url https://.../portrait.jpg \
      --script "Hi, I'm your Mocombe agent." [--voice_sample_url https://.../voice.wav] --out agent.mp4

  # generative text-to-video (same engine as the Mocombe product clips)
...
def main():
    ...
    a = sub.add_parser("avatar", help="talking-avatar (MuseTalk + XTTS)")
